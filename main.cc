...
void simulateQueue(list<packet> &buffer, int discipline)
{
  ...
  while (nbOfTransmissions < maxTransmission)
  {
	...
	cout<<"INFO: Before processing"<<endl;
	printEvents();
    it=events.begin();
	...
	case 0: 
	{
		...
		cout<<"INFO: Before addNextAddrival(), number of Packet in Buffer = "<<buffer.size()<<endl;
		...
		cout<<"INFO: Current number of Packet in Buffer = "<<buffer.size()<<endl;
		break;
	}
	//transmission
	case 1: if(buffer.empty()) break;//If the buffer is empty there is no transmission
		...
		cout<<"INFO: Queue buffer size ="<<buffer.size()<<" start tx time = "<<currentTime<<endl;
		startTxTime = currentTime;
		timeToTransmit = currentTime + transmitNextPackets(0, buffer);
		//scheduleNextTransmission(timeToTransmit);
	 	nbOfTransmissions++;
		stopTxTime = timeToTransmit;
		cout<<"INFO: Queue a packet has been transmitted, buffer size ="<<buffer.size()<<" stop tx time = "<<stopTxTime<<endl;
		break; }
